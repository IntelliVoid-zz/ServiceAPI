...
class Service:
	...
	def addQuota(self, APIKey, Quota = 500):
		"""Adds quota to a API Key
		
		Arguments:
			APIKey {[type]} -- The public API Key from the consumer
		
		Keyword Arguments:
			Quota {[type]} -- The amount of quota to add to the key (default: {500})
		"""

		self.RequestHandler.sendRequest({'action': 'addQuota', 'service_authkey': self.ServiceAuthKey, 'apiKey': APIKey, 'quota': Quota}, True)
		return(True)